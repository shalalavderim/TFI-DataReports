...
from src.reports.t_count_rule import t_count_rule


# This method implements the integration test for transformation t_count_rule
def test_t_count_rule():
    spark_context = SparkContext()
    spark = SparkSession.builder.getOrCreate()
    spark_context.setLogLevel("ERROR")

    print("\n---- Start Test t_count_rule ----")
    print("Reading input data ...")
    input_df = spark.read.parquet("tests/testdata/input_data.parquet")
    input_df.show(5)

    excpeted_df = spark.read.parquet("tests/testdata/expected_data_t1.parquet")
    excpeted_df.show(5)

    print("Applying Transformations ...")
    result_df = t_count_rule(input_df, "R1")
